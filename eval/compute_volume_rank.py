...
METRICS = ("tts_ratio_loudness",)


# Loudness ratio uses negative dB values, so "louder than reference" means a
# smaller ratio (< 1) and "quieter than reference" means a larger ratio (> 1).
def check_ratio_logic(value, style_name):
    s = style_name.lower()
    if "loud" in s or "shout" in s:
        return value < 1
    if "quiet" in s or "whisper" in s:
        return value > 1
    return None
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "audio_dirs",
        nargs="+",
        help="Result directories whose basename contains loud/shout/quiet/whisper (e.g. results/<model>/<location>/en_loud).",
    )
