...
# Style key -> Voxlect accent label that should win the argmax.
# Paper §3.2 evaluates two accents; "English" is kept here only as a target
# label option (it is the argmax bucket for the en_england style if a future
# extension reintroduces that style — see Extending the instruction set in
# the README).
MAPPING = {
...
# When True, restrict the argmax to the target accents defined above.
THREE_WAY = True


def check_following(vals):
    """Return True iff the predicted accent matches MAPPING for the style key."""
...
def run(audio_dirs):
    for dir in audio_dirs:
        ...
        rows = read_metric_file(dir, type="accent")
        rows = [
            r for r in rows
        ]
        ...
        for r in rows:
            ...
            style = r.get("dir", os.path.basename(dir))
        ...
        for turn_id, file_list in turn_to_files.items():
            ...
            for basename in file_list:
                vals = {style: row["accent"] for style, row in data[basename].items()}
                total += 1
        ...
        for turn_id in sorted(turn_stats):
            ...
            print(f"{turn_id}: acc={acc:2.2f} ({p}/{tot})", end="  |  ")
